...
def import_chunk(database, data, module):
...
def import_file(database, path, module, chunk_size):
    ...
    for chunk in progressbar.progressbar(grouper(reader, chunk_size), max_value=line_count(path)/chunk_size):
        import_chunk(database, chunk, module)
...
def main():
    ...
    database = init_db()
    import_file(database, "weatherstats_calgary_daily.csv", import_daily, 1024)
    import_file(database, "weatherstats_calgary_hourly.csv", import_hourly, 1024)
